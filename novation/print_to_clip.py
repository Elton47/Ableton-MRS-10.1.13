    ...
    def _handle_data_packet(self, data_bytes):
        ...
        if len(payload) == BYTES_PER_GROUP_OFFSET:
            ...
            if payload_length % BYTES_PER_NOTE == 0:
                self._clip_data['notes'].extend([ create_note(payload[i:i + BYTES_PER_NOTE], group_offset) for i in xrange(0, payload_length, BYTES_PER_NOTE)
                                                ])
    ...
    def _print_data_to_clip(self):
        if 'length' in self._clip_data:
            ...
            if liveobj_valid(clip):
                ...
                clip.set_notes(tuple(sorted(self._clip_data['notes'], key=itemgetter(1))))
