...
class LaunchkeyElements(object):
    ...
    def __init__(self, *a, **k):
        ...
        self.clip_launch_matrix = ButtonMatrixElement(rows=[ [ create_button(offset + col_index, ('{}_Clip_Launch_Button_{}').format(col_index, row_index), msg_type=MIDI_NOTE_TYPE, channel=0) for col_index in xrange(SESSION_WIDTH) ] for row_index, offset in enumerate(xrange(96, 119, 16))
        ...
        drum_pad_rows = (
         ...
         (40, 41, 42, 43),
         (36, 37, 38, 39))
        self.drum_pads = ButtonMatrixElement(rows=[ [ create_button(row_identifiers[col_index], ('Drum_Pad_{}_{}').format(col_index, row_index), msg_type=MIDI_NOTE_TYPE, channel=DRUM_CHANNEL) for col_index in xrange(4) ] for row_index, row_identifiers in enumerate(drum_pad_rows)
                                                  ], name='Drum_Pads')
        self.pots = ButtonMatrixElement(rows=[
         [ create_encoder(index + 21, ('Pot_{}').format(index)) for index in xrange(SESSION_WIDTH)
         ]], name='Pots')
