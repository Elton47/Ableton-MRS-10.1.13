...
class Elements(object):
    ...
    def __init__(self, *a, **k):
        ...
        pads_raw = [ [ create_button(offset + col_index, ('Pad_{}_{}').format(col_index, row_index), msg_type=MIDI_NOTE_TYPE) for col_index in xrange(SESSION_WIDTH) ] for row_index, offset in enumerate((96,
                                                                                                                                                                                                           112))
                   ]
        self.pads = ButtonMatrixElement(rows=pads_raw, name='Pads')
        self.pads_quadratic = ButtonMatrixElement(rows=[
         pads_raw[0][4:], pads_raw[1][4:], pads_raw[0][:4], pads_raw[1][:4]], name='Pads_Quadratic')
        self.pads_flattened = ButtonMatrixElement(rows=[
         pads_raw[0] + pads_raw[1]], name='Pads_Flattened')
        self.shifted_pad_row_1 = ButtonMatrixElement(rows=[[ with_shift(control) for control in pads_raw[1] ]], name='Pad_Row_1_With_Shift')
        scene_launch_buttons_raw = [ create_button(83 + row_index, name=('Scene_Launch_Button_{}').format(row_index)) for row_index in xrange(SESSION_HEIGHT)
                                   ]
        self.scene_launch_buttons = ButtonMatrixElement(rows=[
         scene_launch_buttons_raw], name='Scene_Launch_Buttons')
        self.shifted_scene_launch_button_1 = with_shift(scene_launch_buttons_raw[1])
        sliders = [ SpecialFeedbackChannelSliderElement(MIDI_CC_TYPE, DEFAULT_CHANNEL, 41 + index, name=('Slider_{}').format(index)) for index in xrange(SESSION_WIDTH)
                  ]
        ...
        self.slider_leds = ButtonMatrixElement(rows=[
         [ create_rgb_led(index + 54, ('Slider_LED_{}').format(index)) for index in xrange(8)
         ]], name='Slider_LEDs')
        self.mixer_soft_button_row_0 = ButtonMatrixElement(rows=[
         [ create_button(identifier, ('Mixer_Soft_Button_{}_0').format(col_index)) for col_index, identifier in enumerate(xrange(59, 59 + SESSION_WIDTH))
         ]], name='Mixer_Soft_Button_Row_0')
        self.mixer_soft_button_row_1 = ButtonMatrixElement(rows=[
         [ create_button(identifier, ('Mixer_Soft_Button_{}_1').format(col_index)) for col_index, identifier in enumerate(xrange(59 + SESSION_WIDTH, 59 + SESSION_WIDTH * 2))
        ...
        self.select_buttons_raw = [ create_button(51 + index, ('Select_Button_{}').format(index)) for index in xrange(8)
                                  ]
        self.select_buttons = ButtonMatrixElement(rows=[
         self.select_buttons_raw], name='Select_Buttons')
        self.select_buttons_with_shift_raw = [ with_shift(button) for button in self.select_buttons_raw ]
        self.display_layout_switch = SysexElement(name='Display_Layout_Switch', send_message_generator=lambda v: sysex.SET_SCREEN_LAYOUT_MESSAGE_HEADER + (
        ...
        self.color_field_lines_0_1_flattened = ButtonMatrixElement(rows=[
         color_field_line_0_raw + color_field_line_1_raw], name='Color_Field_Lines_0_1_Flattened')
        ...
        self.selection_field_lines_0_1_flattened = ButtonMatrixElement(rows=[
         selection_field_line_0_raw + self.selection_field_line_1_raw], name='Selection_Field_Lines_0_1_Flattened')
        ...
        encoders = [ EncoderElement(MIDI_CC_TYPE, DEFAULT_CHANNEL, 21 + index, map_mode=Live.MidiMap.MapMode.relative_smooth_two_compliment, name=('Encoder_{}').format(index)) for index in xrange(8)
                   ]
        ...
        self.encoder_color_fields = ButtonMatrixElement(rows=[
         [ create_display_color_element(index, 1, ('Encoder_Color_Field_{}').format(index)) for index in xrange(8)
