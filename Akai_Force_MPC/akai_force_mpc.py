...
class Akai_Force_MPC(ControlSurface):
    ...
    def __init__(self, *a, **k):
        ...
        with self.component_guard():
            self._ping_element = SysexElement(send_message_generator=const(SYSEX_MSG_HEADER + (BROADCAST_ID, PING_MSG_TYPE, SYSEX_END_BYTE)), name='Ping_Element')
            self._pong_element = MultiElement(name='Pong_Multi_Element', *[ IdentifyingSysexElement(sysex_identifier=SYSEX_MSG_HEADER + (identifier, PONG_MSG_TYPE), name=('Pong_Element_{}').format(index)) for index, identifier in enumerate(SUPPORTED_PRODUCT_IDS)
                                                                          ])
            self._ping_pong = PingPongComponent(on_pong_callback=self._identify, on_ping_timeout=partial(self._enable_components, False), name='Ping_Pong')
    ...
    def _create_launch_modes(self):
        ...
        self._launch_modes.add_mode('clip_launch', [
         ...
         ExtendComboElementMode(combo_pairs=zip(chain(*self._elements.clip_color_controls_raw), chain(*self._elements.physical_clip_color_controls_raw)))], cycle_mode_button_color='Mode.Off')
        if not self.is_force:
            self._launch_modes.add_mode('scene_launch', [
             partial(self._elements.launch_mode_switch.send_value, 1),
             LayerMode(self._scene_list, Layer(scene_launch_buttons='mpc_scene_launch_buttons', scene_color_controls='mpc_scene_color_controls'))], cycle_mode_button_color='Mode.On')
