...
class Elements(LaunchpadElements):
    ...
    def __init__(self, skin=None, *a, **k):
        ...
        self.button_faders = ButtonMatrixElement(rows=[
         [ create_slider(index + self.button_fader_cc_offset, ('Button_Fader_{}').format(index)) for index in xrange(SESSION_WIDTH)
         ]], name='Button_Faders')
        self.button_fader_color_elements_raw = [ create_button(index + self.button_fader_cc_offset, ('Button_Fader_Color_Element_{}').format(index), channel=BUTTON_FADER_COLOR_CHANNEL) for index in xrange(SESSION_WIDTH)
                                               ]
