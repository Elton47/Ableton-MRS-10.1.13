...
class KompleteKontrolBase(ControlSurface):
    ...
    def _create_controls(self):
        ...
        self._volume_encoders = ButtonMatrixElement(rows=[
         [ create_encoder(index + 80, ('Volume_Encoder_{}').format(index), is_s_mk2=self.is_s_mk2) for index in xrange(NUM_TRACKS)
         ]], name='Volume_Encoders')
        self._pan_encoders = ButtonMatrixElement(rows=[
         [ create_encoder(index + 88, ('Pan_Encoder_{}').format(index), is_s_mk2=self.is_s_mk2) for index in xrange(NUM_TRACKS)
         ]], name='Pan_Encoders')
        self._track_name_displays = ButtonMatrixElement(rows=[
         [ create_display_line(sysex.TRACK_NAME_DISPLAY_HEADER, index, ('Track_Name_Display_{}').format(index), width=33 if self.is_s_mk2 else 11) for index in xrange(NUM_TRACKS)
         ]], name='Track_Name_Displays')
        self._track_volume_displays = ButtonMatrixElement(rows=[
         [ create_display_line(sysex.TRACK_VOLUME_DISPLAY_HEADER, index, ('Track_Volume_Display_{}').format(index), width=12) for index in xrange(NUM_TRACKS)
         ]], name='Track_Volume_Displays')
        self._track_panning_displays = ButtonMatrixElement(rows=[
         [ create_display_line(sysex.TRACK_PANNING_DISPLAY_HEADER, index, ('Track_Panning_Display_{}').format(index), width=12) for index in xrange(NUM_TRACKS)
         ]], name='Track_Panning_Displays')
        self._track_type_displays = ButtonMatrixElement(rows=[
         [ create_sysex_element(sysex.TRACK_TYPE_DISPLAY_HEADER, index, ('Track_Type_Display_{}').format(index)) for index in xrange(NUM_TRACKS)
         ]], name='Track_Type_Displays')
        self._track_mute_displays = ButtonMatrixElement(rows=[
         [ create_sysex_element(sysex.TRACK_MUTE_DISPLAY_HEADER, index, ('Track_Mute_Display_{}').format(index)) for index in xrange(NUM_TRACKS)
         ]], name='Track_Mute_Displays')
        self._track_solo_displays = ButtonMatrixElement(rows=[
         [ create_sysex_element(sysex.TRACK_SOLO_DISPLAY_HEADER, index, ('Track_Solo_Display_{}').format(index)) for index in xrange(NUM_TRACKS)
         ]], name='Track_Solo_Displays')
        self._track_muted_via_solo_displays = ButtonMatrixElement(rows=[
         [ create_sysex_element(sysex.TRACK_MUTED_VIA_SOLO_DISPLAY_HEADER, index, ('Track_Muted_via_Solo_Display_{}').format(index)) for index in xrange(NUM_TRACKS)
         ]], name='Track_Muted_via_Solo_Displays')
        self._track_selection_displays = ButtonMatrixElement(rows=[
         [ create_sysex_element(sysex.TRACK_SELECT_DISPLAY_HEADER, index, ('Track_Selection_Display_{}').format(index)) for index in xrange(NUM_TRACKS)
