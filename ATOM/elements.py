...
class Elements(object):
    ...
    def __init__(self, *a, **k):
        ...
        self.shift_button = create_button(32, 'Shift_Button', resource_type=PrioritizedResource)
        self.zoom_button = create_button(104, 'Zoom_Button', resource_type=PrioritizedResource)
        ...
        self.pads_raw = [ [ create_button(offset + col_index, ('{}_Pad_{}').format(col_index, row_index), msg_type=MIDI_NOTE_TYPE) for col_index in xrange(SESSION_WIDTH) ] for row_index, offset in enumerate(xrange(48, 32, -4))
                        ]
        self.pads = ButtonMatrixElement(rows=self.pads_raw, name='Pads')

        def with_modifier(modifier_button, button):
            return ComboElement(control=button, modifier=modifier_button, name=('{}_With_{}').format(button.name, modifier_button.name.split('_')[0]))
        ...
        self.encoders = ButtonMatrixElement(rows=[
         [ create_encoder(index + 14, ('Encoder_{}').format(index)) for index in xrange(4)
         ]], name='Encoders')