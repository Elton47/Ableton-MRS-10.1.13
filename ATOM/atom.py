...
class ATOM(ControlSurface):
    ...
    def _create_lighting(self):
        self._lighting = LightingComponent(name='Lighting', is_enabled=False, layer=Layer(shift_button='shift_button', zoom_button='zoom_button'))
        self._lighting.set_enabled(True)

    def _create_transport(self):
        self._transport = TransportComponent(name='Transport', is_enabled=False, layer=Layer(play_button='play_button', loop_button='play_button_with_shift', stop_button='stop_button', metronome_button='click_button'))
    ...
    def _create_undo(self):
        self._undo = UndoRedoComponent(name='Undo', is_enabled=False, layer=Layer(undo_button='stop_button_with_shift'))
        self._undo.set_enabled(True)

    def _create_view_toggle(self):
        self._view_toggle = ViewToggleComponent(name='View_Toggle', is_enabled=False, layer=Layer(detail_view_toggle_button='show_hide_button', main_view_toggle_button='preset_button'))
        self._view_toggle.set_enabled(True)

    def _create_background(self):
        self._background = BackgroundComponent(name='Background', is_enabled=False, add_nop_listeners=True, layer=Layer(set_loop_button='set_loop_button', nudge_button='nudge_button', bank_button='bank_button'))
        self._background.set_enabled(True)

    def _create_session(self):
        self._session_ring = SessionRingComponent(name='Session_Ring', num_tracks=SESSION_WIDTH, num_scenes=SESSION_HEIGHT)
        self._session = SessionComponent(name='Session', session_ring=self._session_ring)
        self._session_navigation = SessionNavigationComponent(name='Session_Navigation', is_enabled=False, session_ring=self._session_ring, layer=Layer(left_button='left_button', right_button='right_button'))
        self._session_navigation.set_enabled(True)
        self._session_overview = SessionOverviewComponent(name='Session_Overview', is_enabled=False, session_ring=self._session_ring, enable_skinning=True, layer=Layer(button_matrix='pads_with_zoom'))

    def _create_mixer(self):
        self._mixer = MixerComponent(name='Mixer', auto_name=True, tracks_provider=self._session_ring, track_assigner=SimpleTrackAssigner(), invert_mute_feedback=True, channel_strip_component_type=ChannelStripComponent)
    ...
    def _create_session_navigation_modes(self):
        self._session_navigation_modes = ModesComponent(name='Session_Navigation_Modes', is_enabled=False, layer=Layer(cycle_mode_button='bank_button'))
        self._session_navigation_modes.add_mode('default', AddLayerMode(self._session_navigation, layer=Layer(up_button='up_button', down_button='down_button')), cycle_mode_button_color='DefaultButton.Off')
        self._session_navigation_modes.add_mode('paged', AddLayerMode(self._session_navigation, layer=Layer(page_up_button='up_button', page_down_button='down_button', page_left_button='left_button', page_right_button='right_button')), cycle_mode_button_color='DefaultButton.On')
        self._session_navigation_modes.selected_mode = 'default'

    def _create_keyboard(self):
        self._keyboard = KeyboardComponent(midi.KEYBOARD_CHANNEL, name='Keyboard', is_enabled=False, layer=Layer(matrix='pads', scroll_up_button='up_button', scroll_down_button='down_button'))

    def _create_drum_group(self):
        self._drum_group = DrumGroupComponent(name='Drum_Group', is_enabled=False, translation_channel=midi.DRUM_CHANNEL, layer=Layer(matrix='pads', scroll_page_up_button='up_button', scroll_page_down_button='down_button'))
    ...
    def _create_pad_modes(self):
        self._pad_modes = ModesComponent(name='Pad_Modes', is_enabled=False, layer=Layer(session_button='full_level_button', note_button='note_repeat_button', channel_button='select_button', encoder_modes_button='setup_button'))
        self._pad_modes.add_mode('session', (
         AddLayerMode(self._background, Layer(unused_pads='pads_with_shift')),
         AddLayerMode(self._session, Layer(clip_launch_buttons='pads', scene_launch_buttons=self._elements.pads_with_shift.submatrix[
          3:, :])),
